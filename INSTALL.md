...
адресуются `ROLE-PROCESS-OWNER`, а если не назначен и он сам — `ROLE-PRODUCT-OWNER` как
роли, которая инициировала работу. Без ответа такие вопросы остаются открытыми навсегда,
блокируя всё, что от них зависит.
...
Если не назначен и сам `ROLE-PROCESS-OWNER` — то есть нет роли, которая назначает роли, —
вопрос уходит `ROLE-PRODUCT-OWNER`: он инициировал работу и вправе поставить вопрос о
составе ролей перед теми, кто его формирует. Довести находку в этом случае нужно до
владельца продукта, а не до владельца процесса, которого нет.
