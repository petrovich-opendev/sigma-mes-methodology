...
Чего проверка НЕ делает: она ищет признак правила рядом с маркером, а не сверяет перечни
между носителями поэлементно. Расхождение вида «здесь четыре пункта, там пять» она не
увидит, если требуемый признак есть в обоих. Ограничение осознанное: надёжно сверять
списки в текстах разной формы — таблица, маркированный список, проза внутри
нумерованного пункта — не получается.
"""
import re, sys
